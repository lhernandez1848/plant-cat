...
export default function Accordion({ question, answer }: { question: string; answer: string }) {
  ...
  return (
    <div className="border border-gray-300 rounded-lg mb-4">
      <button onClick={() => handleClick()} className={filterClassNames(isOpen && "bg-teal-100","flex items-center justify-between w-full p-5 font-medium rtl:text-right text-gray-500 border border-b-0 border-gray-200 rounded-t-xl hover:bg-teal-100 gap-3")}>
        <h4>{question}</h4>
