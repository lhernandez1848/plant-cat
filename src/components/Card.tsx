...
export default function Card(plant : PlantProps) {
  ...
  return (
    <a href={`/details/${plant.id}`} className="w-full h-full flex flex-col transition-transform duration-300 hover:scale-105">
      <img className="rounded-t-lg mb-2 w-full h-40 object-cover" src={plant.image_url ? plant.image_url : '/assets/default-image.jpg'} alt={plant.common_name} />
      <h5 className="text-lg md:text-xl lg:text-2xl title-font text-gray-800 break-all">{capitalizeFirstLetter(plant.common_name)}</h5>
