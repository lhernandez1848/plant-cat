  ...
  const navMenu = navigation.map((item) => (
    <li key={item.name}>
      ...
      className={classNames(
        item.current ? 'text-teal-800 md:text-teal-800 md:p-0' : 'hover:bg-gray-100 md:hover:bg-transparent md:hover:text-teal-800 md:p-0', 'text-sm font-semibold text-gray-900 rounded-sm',
      )}>{item.name}</Link>
    </li>
  ))
  ...
  return <nav className="bg-white sticky w-full z-2 border-b border-teal-700">
      <div className="max-w-screen-xl flex flex-wrap items-center justify-between mx-auto p-4">
        ...
        <div className="flex md:order-2">
          <button type="button" data-collapse-toggle="navbar-search" aria-controls="navbar-search" aria-expanded="false" className="sm:hidden text-gray-500 hover:bg-gray-100 focus:outline-none focus:ring-4 focus:ring-gray-200 rounded-lg text-sm p-2.5 me-1">
          ...
          <div className="relative hidden sm:block pr-3">
            ...
            <input type="text" id="search-navbar" className="block w-full p-2 ps-10 text-sm text-gray-900 border border-teal-400 rounded-lg bg-gray-50 focus:ring-blue-500 focus:border-blue-500" placeholder="Search..." />
          </div>
      ...
      {isOpen &&
        <div className="md:hidden" id="mobile-menu">
          <div className="space-y-1 px-2 pt-2 pb-3">
            <ul className="flex flex-col p-2 font-medium border border-gray-100 rounded-lg bg-gray-50 md:bg-white">
              {navMenu}
            </ul>
          </div>
        </div>
