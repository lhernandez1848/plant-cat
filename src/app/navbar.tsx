  ...
  const navMenu = navigation.map((item) => (
    <li key={item.name}>
      ...
      className={filterClassNames(
        item.current ? 'text-emerald-800 md:text-emerald-800 md:p-0' : 'hover:bg-gray-100 md:hover:bg-transparent md:hover:text-emerald-800 md:p-0', 'text-gray-900 rounded-sm',
      )}>{item.name}</Link>
    </li>
  ))

  return <nav className="bg-white sticky w-full z-2 border-b border-emerald-700">
      <div className="max-w-screen-xl flex flex-wrap items-center justify-between mx-auto p-4">
        <Link href="/" className="flex items-center space-x-3 rtl:space-x-reverse">
          <Image className="h-8"
            ...
            height={38}
            priority />
            <span className="self-center text-4xl text-emerald-800 font-semibold whitespace-nowrap title-font">Plant Cat</span>
        </Link>
